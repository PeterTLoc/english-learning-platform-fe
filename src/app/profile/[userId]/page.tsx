...
import { User, BookOpen, Calendar, Mail, Edit2, Save, GraduationCap, Layers, Star, KeyRound, Eye, EyeOff } from "lucide-react";
...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const flashcardSetsResponse = await flashcardSetService.getFlashcardSets(
          1,
          100,
          "",
          "createdAt",
  ...
  useEffect(() => {
    if (selectedSection === "Flashcards" || selectedSection === "Achievements") {
                ...
                <div className="flex-shrink-0">
                  ...
                  {avatarFile && (
                    <div className="mt-2 text-center">
                      <span className="text-xs text-gray-400">{avatarFile.name}</span>
                    </div>
                  )}
                </div>
                {/* User Info */}
            ...
            {selectedSection === "Courses" && (
              ...
              {loadingCourses ? (
                <div className="flex flex-col items-center gap-4 justify-center py-12">
                  <LoadingSpinner />
                  <span className="ml-2 text-[#CFCFCF]">Loading courses...</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userCourses.map((uc) => (
                    <div key={uc._id} className="bg-[#2b2b2b] hover:scale-105 rounded-lg shadow p-5 flex flex-col gap-4">
                      ...
                      <div className="flex flex-col gap-2 flex-1">
                        <h3 className="text-lg font-bold text-white mb-1 truncate">{uc.course?.name || "Unknown Course"}</h3>
                        <div className="flex items-center gap-2 text-xs text-gray-400 mb-2">
                          <span className="bg-blue-900/40 text-blue-300 px-2 py-0.5 rounded">{uc.course?.level || 'Unknown Level'}</span>
                          <span className="bg-purple-900/40 text-purple-300 px-2 py-0.5 rounded">{uc.course?.type || 'Unknown type'}</span>
                        </div>
                        <div className="flex items-center gap-2 text-sm text-gray-300 mb-2">
                          <span>Progress:</span>
                          <span className="font-semibold text-white">
                            {uc.lessonFinished} / {uc.course?.totalLessons || 0} lessons
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <span className={`px-2 py-0.5 rounded-full font-semibold ${uc.status === "completed" ? "bg-green-700 text-green-200" : uc.status === "ongoing" ? "bg-yellow-700 text-yellow-200" : "bg-gray-700 text-gray-300"}`}>{capitalizeStatus(uc.status)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Lessons */}
                <div>
                  <h3 className="text-xl font-semibold text-blue-300 mb-4">Lessons</h3>
                  {loadingLessons ? (
                    <div className="flex flex-col items-center gap-4 justify-center py-8">
                      <LoadingSpinner />
                      <span className="ml-2 text-[#CFCFCF]">Loading lessons...</span>
                    </div>
                  ) : userLessons.length === 0 ? (
                    <div className="bg-[#2b2b2b] rounded-lg p-4 text-gray-400">No lessons found.</div>
                  ) : (
                    <div className="space-y-4">
                      {userLessons.map((ul) => (
                        <div key={ul._id} className="bg-[#2b2b2b] rounded-lg p-4 flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-white">{ul.lesson?.name || "Unknown Lesson"}</span>
                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ml-2 ${ul.status === "completed" ? "bg-green-700 text-green-200" : ul.status === "in-progress" ? "bg-yellow-700 text-yellow-200" : "bg-gray-700 text-gray-300"}`}>{capitalizeStatus(ul.status)}</span>
                          </div>
                ...
                {/* Tests */}
                <div>
                  <h3 className="text-xl font-semibold text-pink-300 mb-4">Tests</h3>
                  {loadingTests ? (
                    <div className="flex flex-col items-center gap-4 justify-center py-8">
                      <LoadingSpinner />
                      <span className="ml-2 text-[#CFCFCF]">Loading tests...</span>
                    </div>
                  ) : userTests.length === 0 ? (
                    <div className="bg-[#2b2b2b] rounded-lg p-4 text-gray-400">No tests found.</div>
                  ) : (
                    <div className="space-y-4">
                      {userTests.map((ut) => (
                        <div key={ut._id} className="bg-[#2b2b2b] rounded-lg p-4 flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-white">{ut.test?.name || "Unknown Test"}</span>
                            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ml-2 ${ut.status === "completed" ? "bg-green-700 text-green-200" : ut.status === "in-progress" ? "bg-yellow-700 text-yellow-200" : "bg-gray-700 text-gray-300"}`}>{capitalizeStatus(ut.status)}</span>
                          </div>
                          <div className="text-gray-400 text-sm">
                            Score: <span className="text-white font-semibold">{ut.score}</span> | Last updated: {formatDate(ut.updatedAt)}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {loadingUserDetail ? (
                <div className="flex flex-col items-center gap-4 justify-center py-12">
                  <LoadingSpinner />
                  <span className="ml-2 text-[#CFCFCF]">Loading flashcard sets...</span>
                </div>
              ) : (
                <>
                  <h3 className="text-lg font-semibold text-white mb-4">Your Flashcard Sets</h3>
                  {flashcardSets.length === 0 ? (
                    <div className="bg-[#2b2b2b] rounded-lg p-4 text-gray-400">No flashcard sets found.</div>
                  ) : (
                    <div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {flashcardSets.map((set) => (
                          <FlashcardSetCard key={set._id as string} flashcardSet={set} />
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {loadingUserDetail ? (
                <div className="flex flex-col items-center gap-4 justify-center py-12">
                  <LoadingSpinner />
                  <span className="ml-2 text-[#CFCFCF]">Loading achievements...</span>
                </div>
              ) : userDetail?.achievements?.list?.length ? (
                <div className="space-y-4">
                  {userDetail.achievements.list.map((ach: any) => (
                    <div key={ach._id} className="bg-[#2b2b2b] hover:scale-105 rounded-lg p-4 flex flex-col gap-2">
                      <span className="font-bold text-white text-lg">{ach.title}</span>
                      <span className="text-gray-300 text-sm">{ach.description}</span>
                      <span className="text-xs text-gray-400">Awarded: {formatDate(ach.dateAwarded)}</span>
                    </div>
                  ))}
                </div>
              ) : (

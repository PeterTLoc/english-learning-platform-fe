...
export default function CourseFormModal({
  ...
  const [selectedLessons, setSelectedLessons] = useState<string[]>(data?.lessonIds || []);
  ...
  const [answers, setAnswers] = useState<string[]>(
    data?.answer ? Array.isArray(data.answer) ? data.answer : [data.answer] : ['']
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (type === 'lesson') {
        ...
        if (!formState.name || !formState.description || !formState.courseId || !formState.totalQuestions) {
          throw new Error('Name, description, course ID, and total questions are required for tests');
        }
        ...
        if (!formState.question || !formState.type || !formState.focus || !formState.lessonId) {
          throw new Error('Question, type, focus, and lesson ID are required for exercises');
        }

        // Validate answers
        if (formState.type === ExerciseTypeEnum.MULTIPLE_CHOICE) {
          if (options.length < 2) {
            throw new Error('Multiple choice exercises require at least 2 options');
          }
        ...
        if (mode === 'edit' && !imageFile && !shouldDeleteImage && (data?.image || data?.exerciseImage)) {
      ...
      <div>
        ...
        <div className="max-h-40 overflow-y-auto bg-[#2D2D2D] border border-[#1D1D1D] rounded-md p-2">
          {!courseDetails?.lessons.length ? (
            <p className="text-[#9A9A9A] text-sm p-2">No lessons available in this course</p>
          ) : (
            courseDetails.lessons.map(({ lesson }, index) => (
              <div key={lesson._id} className="flex items-center space-x-2 py-2 hover:bg-[#373737] px-2 rounded">
                <input
                  type="checkbox"
                ...
                <div className="flex-1">
                  ...
                  >
                    <span>Lesson {index + 1}: {lesson.name}</span>
                  ...
                  {lesson.description && (
                    <p className="text-sm text-[#9A9A9A] mt-1 line-clamp-2">{lesson.description}</p>
                  )}
                </div>
              </div>
            ))
        ...
        <p className="text-[#9A9A9A] text-sm mt-1">
          Selected: {selectedLessons.length} lesson{selectedLessons.length !== 1 ? 's' : ''}
        </p>
      </div>
      <div>
        ...
        <input
          ...
          onChange={(e) => setFormState({ ...formState, totalQuestions: e.target.value })}
          className="w-full p-2 bg-[#2D2D2D] border border-[#1D1D1D] rounded-md text-white"
          min="1"
          required
        />
  ...
  const renderExerciseForm = () => (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-[#CFCFCF] mb-1">Question <span className="text-red-500">*</span></label>
        <textarea
      ...
      <div>
        <label className="block text-[#CFCFCF] mb-1">Type <span className="text-red-500">*</span></label>
        <select
      ...
      <div>
        <label className="block text-[#CFCFCF] mb-1">Focus <span className="text-red-500">*</span></label>
        <select
      ...
      {formState.type !== ExerciseTypeEnum.MULTIPLE_CHOICE && (
        <div>
          <label className="block text-[#CFCFCF] mb-2">Answers <span className="text-red-500">*</span></label>
      ...
      {formState.type === ExerciseTypeEnum.MULTIPLE_CHOICE && (
        <div>
          <label className="block text-[#CFCFCF] mb-1">Answer <span className="text-red-500">*</span></label>
          <select
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              required={mode === "create" && formState.type === ExerciseTypeEnum.IMAGE_TRANSLATE}
            />
          </div>
          {imagePreview && (
        ...
        <div className="p-6 border-b border-[#1D1D1D]">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-white">
              {mode === "edit" ? "Edit Exercise" : "Create Exercise"}
            </h2>
            <button onClick={onCancel} className="text-[#CFCFCF] hover:text-white">

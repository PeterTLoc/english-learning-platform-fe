...
export default function CourseDetail({ course }: CourseDetailProps) {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        const data = await getAllLessonsByCourseId(course._id.toString());
        setLessons(data);
  ...
  const renderContent = () => {
    ...
    if (
      currentLesson &&
      currentView.contentType === LESSON_CONTENT_TYPES.OVERVIEW
    ) {
      return (
        <LessonOverview
      ...
      <div className="px-5 flex-1 mt-5">
        <div className="max-w-[1000px] mx-auto">
          <div className="flex items-center justify-between mb-3">
            <h1 className="title">{getBreadcrumbTitle()}</h1>

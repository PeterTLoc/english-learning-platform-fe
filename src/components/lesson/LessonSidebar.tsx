      ...
      try {
        ...
        for (const lesson of memoizedLessons) {
          ...
          const completedExercises = userExercises.filter(
            (userExercise: UserExercise) => userExercise.completed
          );

          completionStatus[lesson._id] =
        ...
        try {
          ...
          const availableTests = await getTestsByLessonId(
            memoizedLessons[0]?._id || ""
          );
          ...
          if (availableTests.length === 0) {
            ...
            userTests.forEach((userTest: UserTest) => {
            ...
            const allAvailableTestIds = availableTests.map(
              (test: { _id: string }) => test._id
            );
            allTestsPassed = allAvailableTestIds.every((testId) =>

...
export const getUserLessonById = async (userLessonId: string): Promise<UserLesson> => {
...
export const updateUserLesson = async (id: string, data: Partial<UserLesson>) => {
  const response = await api.put(`/api/user-lessons/${id}`, data)
  return response.data
}
